(ns forestrylabour.actor-test
  (:require [clojure.test :refer [deftest is testing]]
            [forestrylabour.actor :as actor]
...
(deftest commits-a-registered-work-log
  (let [st (fresh-store)
        ...
        request {:worker-id "worker-1" :op :log-work-record :stake :low
                  :site-id "SITE-1" :task "brush-clearing progress log"}
...
(deftest holds-an-unregistered-site-proposal
  (let [st (fresh-store)
        graph (actor/build-graph {:store st})
        request {:worker-id "worker-1" :op :log-work-record :stake :low
                  :site-id "SITE-ghost" :task "brush-clearing progress log"}
...
(deftest interrupts-then-approves-safety-concern-on-human-approval
  (let [st (fresh-store)
        ...
        request {:worker-id "worker-1" :op :flag-safety-concern :stake :low
                  :site-id "SITE-1" :hazard-type :falling-hazard}
...
(deftest holds-a-scope-excluded-forestry-work-execution-op-even-at-high-confidence
  (testing "an actor run can never commit a proposal that would finalize a forestry-work-execution decision, regardless of disposition path"
    (let [st (fresh-store)
          graph (actor/build-graph {:store st})
          request {:worker-id "worker-1" :op :authorize-felling-operation :stake :low
                    :site-id "SITE-1" :task "felling authorization decision"}
  ...
  (testing "an actor run can never commit a proposal that would finalize a site-safety-clearance decision (e.g. declaring a site cleared for safety), regardless of disposition path"
    (let [st (fresh-store)
          ...
          request {:worker-id "worker-1" :op :declare-site-safety-cleared :stake :low
                    :site-id "SITE-1" :task "site safety clearance decision"}
