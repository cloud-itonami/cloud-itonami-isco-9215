(ns forestrylabour.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [forestrylabour.store :as store]
            [forestrylabour.advisor :as advisor]
            [forestrylabour.governor :as governor]))

(defn- fresh-store []
  (let [st (store/mem-store)]
    (store/register-worker! st {:worker-id "worker-1" :name "Kenji Sato"})
    (store/register-site! st {:site-id "SITE-1" :name "Sato Forestry Site" :max-supply-cost 2000})
    st))
...
(deftest hard-on-scope-excluded-op-authorize-felling-operation
  (testing "authorizing a felling operation (a forestry-work-execution decision) to proceed is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :authorize-felling-operation) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-op-authorize-clearing-operation
  (testing "authorizing a clearing operation to proceed is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :authorize-clearing-operation) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest hard-on-scope-excluded-rationale-authorize-felling-operation
  (testing "defense-in-depth: a rationale that itself attempts to authorize the felling operation to proceed is blocked even if the op looks routine"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :schedule-crew-operation)
                                           :rationale "recommend we authorize the felling operation to proceed now")
                             st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest always-escalates-safety-concern-even-at-high-confidence
  (testing "a falling-tree/branch hazard, uneven-terrain hazard, equipment-condition or weather-exposure concern always requires human sign-off"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :flag-safety-concern :hazard-type :falling-hazard)
                                           :confidence 0.99)
                             st)]
      (is (not (:hard? v)))
      (is (:escalate? v)))))
...
(deftest default-mock-advisor-proposals-never-self-trip-on-scope-exclusion
  (testing "the governor's scope-exclusion term list must never match the mock advisor's own default rationale text for any allowlisted op — CLAUDE.md's known self-tripping bug pattern (rationale legitimately contains bare nouns like 'tree'/'branch'/'chainsaw'/'site'/'site safety supervisor', but never the full finalization-action phrases)"
    (let [st (fresh-store)
      ...
      (doseq [o ops]
        (let [request {:worker-id "worker-1" :op o :site-id "SITE-1"
                        :stake :low :task "routine forestry site maintenance task"
                        :hazard-type :falling-hazard :cost 500}
