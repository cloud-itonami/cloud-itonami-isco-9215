(ns forestrylabour.advisor
  "Forestry Labourer Advisor — proposing a forestry-site scheduling/
  logistics coordination operation (log a task/progress record, schedule
  a crew operation, flag a safety concern, coordinate a forestry-supplies
  ...
  `forestrylabour.governor` independently gates every proposal and always
  escalates safety concerns and above-threshold supply orders. The
  advisor never proposes to directly finalize a forestry-work-execution
  ...
  safety), and never proposes to override a site safety supervisor's
...
(defn- rationale-for [op worker-id site-id hazard-type]
  (case op
    ...
    (str "scheduled crew operation for forestry support task at site " site-id)
...
(def ^:private system-prompt
  "You are a forestry-site scheduling/logistics coordination advisor.
   ...
   propose to directly finalize a forestry-work-execution decision (e.g.
   authorizing a felling or clearing operation to proceed), or a site-
   safety-clearance decision (e.g. declaring a site cleared for safety),
   or to override a site safety supervisor's judgment — those are always
   out of this actor's scope; it coordinates site scheduling/logistics
   only and never performs forestry work itself or makes site-safety-
